...
public class StoreDao {
	...
	public int productInsert(StoreInfo si) {
		...
		String sql = "insert into t_store_info (si_id, sc_id, si_name, si_img1, si_img2, si_img3, si_star, si_week, si_open, si_close, si_parking, si_addr1, si_addr2, si_lat, si_lng, si_number, si_explan, si_isview, ai_idx) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)";
		int result = jdbc.update(sql, 
				si.getSc_id() + String.format("%03d", randomValue), si.getSc_id(), si.getSi_name(), si.getSi_img1(), si.getSi_img2(), 
				si.getSi_img3(), si.getSi_star(), si.getSi_week(), si.getSi_open(), si.getSi_close(), si.getSi_parking(), si.getSi_addr1(), 
				si.getSi_addr2(), si.getSi_lat(), si.getSi_lng(), si.getSi_number(), si.getSi_explan(), si.getSi_isview()
		);
